...
def main(_config):
    ...
    model.to(device)
    lst_imgs = [f"/home/ali/ViLT/assets/database/{i}.jpg" for i in range(1,6)]
    def infer(url, mp_text, hidx):
        ...
        batch = {"text": [""], "image": imgs}
        tl = len(re.findall("\[MASK\]", mp_text))
        inferred_token = [mp_text]*5
        # batch["image"][0] = img
        ...
        selected_token = ""
        encoded = tokenizer(inferred_token)
        outs = [infer["itm_loss"],infer["itm_wpa_loss"],infer["itm_logits"],infer["itm_labels"]]
        print("outs ",outs)
        ...
        return [np.array(image), inferred_token[0], selected_token,outs[0][0]]
